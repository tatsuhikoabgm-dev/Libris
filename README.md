...
- 2025-11-26: 日本語で処理フローが書けないｗページDTOを作るメソッド内でAPIを叩きＤＢからステータスを取得しそれらをページDTOに詰め込む。
この処理を日本語で書こうとするとAPIから情報を取得→ステータスをDBから取得→ページDTOに詰める、になっちゃう。ifもアクティブのフラグ立てるときにだけ書くのにelseの分も日本語で書かなきゃとかなっちゃうｗ
